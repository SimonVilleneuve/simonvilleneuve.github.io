...
A.A15 = {
  ...
  headIsShown: false,
  changeHead: function(show) {
    if (this.headIsShown == show) return;
    else this.headIsShown = show;
    jQuery('.popup-menu').attr('data-show-on', show ? '' : null);
  },
  ...
  dynoHead: function() { // using timers for high performance:
    ; (function(window, D, $, t){ 'use strict';
      ...
      var handleScroll = function() {
        ...
        var isSafe = st > safeZone;
		var isBeforePageEnd = !(st + $(window).height() >= docH() - safeZone);

		var isHotSpot = mY && mY >= $(window).height() - t.headSafeZone;
		if (isHotSpot) return;
		
		t.changeHead(isSafe && isBeforePageEnd);

        lastTop = st;
      };
	  
      var mouseTimer = null, mTicks = 250;
      ...
      var handleMouse = function(e) {
        ...
        mY = e.clientY;
		if ($(window).scrollTop() <= t.headSafeZone || $(window).scrollTop() + $(window).height() >= docH()) {
			var isHotSpot = mY && mY >= $(window).height() - t.headSafeZone;
			t.changeHead(isHotSpot);
		}
      };
	  
	  // set attribute if the document height is equal to the window height (no scroll)
	  jQuery('html').attr('data-no-scroll', docH() === $(window).height() ? '' : null);
...
/*--------------------------------------------------------------------------
 Global Click Hook for External Links
 /*------------------------------------------------------------------------*/
$(document).on('click', 'a[rel*="external"],a[href^="http"]', function() {
